...
# instrument.server.mjs is loaded via `node --import` outside of Nitro's bundling,
# so its imports (e.g. @sentry/tanstackstart-react) must be resolvable from
# node_modules at runtime; install only production dependencies for that.
...
CMD ["node", "--import", "./instrument.server.mjs", "./index.mjs"]
